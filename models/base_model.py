...
class BaseGenerator(nn.Module):
    def __init__(self,n_res=4,use_bias=False):
        ...
        self.up_sampling = nn.Sequential(
            ...
            nn.ConvTranspose2d(128, 64, kernel_size=5, stride=2, padding=1, output_padding=1, bias=use_bias),
            nn.BatchNorm2d(64,affine=True),
            nn.LeakyReLU(0.2,inplace=True),

            nn.Conv2d(64, 3, kernel_size=5, stride=1, padding=0, bias=use_bias),
            nn.Tanh()
        )
...
class NonResizing_GrayGenerator(nn.Module):
    def __init__(self,n_res=4,use_bias=False):
        super().__init__()
...
class BaseDiscriminator(nn.Module):
    def __init__(self, leaky_relu_negative_slope=0.2, use_bias=False):
        ...
        self.layers = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=5, stride=1, padding=1, bias=use_bias), # 256
...
class GrayDiscriminator(nn.Module):
    def __init__(self,use_bias=False):
        super().__init__()
